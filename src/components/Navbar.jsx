...
import LoginChoiceModal from './LoginChoiceModal'; // <-- IMPORTED THE MODAL

function Navbar({ isAuthenticated, onLogout, user }) {
  const [isMobileMenuOpen, setIsMobileMenuOpen] = useState(false);
  const [isLoginModalOpen, setIsLoginModalOpen] = useState(false); // <-- NEW STATE FOR MODAL
  ...
  const handleLoginClick = (event) => {
    ...
    setIsMobileMenuOpen(false); // Ensure mobile menu closes if action originated there
  };
  ...
  return (
    <>
          ...
          <div className="flex justify-between items-center h-20">
            ...
            <div className="hidden md:flex items-center gap-3">
              {isAuthenticated ? (
                ...
                <>
                  {/* DESKTOP LOGIN LINK - NOW OPENS MODAL */}
            ...
            <div className="md:hidden py-4 border-t border-blue-100 bg-white/95 backdrop-blur-sm">
              ...
              <div className="border-t border-blue-100 mt-4 pt-4 px-4 space-y-3">
                {isAuthenticated ? (
                  ...
                  <>
                    {/* MOBILE LOGIN LINK - NOW OPENS MODAL */}
                    <Link to="/login">
                      <Button
                        variant="outline"
                        className="w-full border-blue-200 text-blue-700 hover:bg-blue-50"
                        onClick={handleLoginClick} // Use the dedicated handler
                      >
                        Login
                      </Button>
                    </Link>
      ...
      {/* LOGIN CHOICE MODAL - RENDERED OUTSIDE NAV */}
      <LoginChoiceModal
